...
| Vector Search | SQLite + numpy cosine similarity + hybrid keyword salience | Local `.db` file (89 files, 625 chunks) |
| Standardized Crop Schema | **5-Section Standard** across all 41 crops | Every crop document strictly implements: (A) Water Requirement by Phase, (B) Fertilizer Requirement by Phase, (C) Total Crop Duration, (D) Harvesting Details, (E) Growing Season |
| Sub-Topic & Intent Layer | Intent-Keyword Mapping (`_SUBTOPIC_RULES` + `_INTENT_KEYWORD_MAP`) | Tags every chunk with sub-topic (irrigation, fertilizer, duration, harvest, season, sowing, pest, disease, yield, eligibility, cost) and applies sub-topic (+0.20), crop-specificity (+0.25), and stage-matching (+0.50) bonuses |
...
| Generalization Benchmark | `scripts/eval_rag.py` | Automated test suite verifying 22 novel 5-section question phrasings across 41 crops (**100% pass rate**) |
...
/gyansetu
  ...
  /scripts           → reindex_all.py, eval_rag.py, generate_all_crop_docs.py
  PRD.md
  TECH_STACK.md
  BRAIN.md
...
On-Device Search      → SQLite + Hybrid Vector & Intent-Keyword Search (89 files, 625 chunks)
Standardized Schema   → 5-Section Standard (Water, Fertilizer, Duration, Harvest, Season) across 41 crops
